...
import type { AiOperation, AiOperationOptions } from '../../../shared/domain/ai.js'
import { ApiError } from '../../api/client.js'
import { getAiOperationOptions } from './resumeEnhancerApi.js'

export type OperationOptionsStatus = 'loading' | 'ready' | 'error'

/**
 * Loads the configured providers and their models for one AI operation
 * (ADR-006).
 *
 ...
 */
export function useAiOperationOptions(operation: AiOperation): {
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      setOptions(await getAiOperationOptions(operation))
      setStatus('ready')
    } catch (loadError) {
      setError(
